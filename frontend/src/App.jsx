...
function parseMarkdownToHTML(markdown) {
  ...
  html = html.replace(
    /^### (.*$)/gim,
    '<h3 style="margin-top:16px; margin-bottom:8px; font-size:16px; font-weight:700; color:#0f172a;">$1</h3>',
  );
  html = html.replace(
    /^## (.*$)/gim,
    '<h2 style="margin-top:20px; margin-bottom:10px; font-size:18px; font-weight:800; color:#0f172a; border-bottom:1px solid #e2e8f0; padding-bottom:6px;">$1</h2>',
  );
  html = html.replace(
    /^# (.*$)/gim,
    '<h1 style="margin-top:24px; margin-bottom:12px; font-size:22px; font-weight:800; color:#0f172a;">$1</h1>',
  );
  html = html.replace(
    /\*\*(.*?)\*\*/gim,
  ...
  html = html.replace(
    /^\s*\-\s+(.*$)/gim,
    '<li style="margin-left:20px; margin-bottom:4px; color:#334155;">$1</li>',
  );
  html = html.replace(/\n\n/gim, "<br/><br/>");
  return html;
}
    ...
    try {
      if (intent === "write_ship30_essay") {
        ...
        const artRes = await fetch(
          `${API_BASE}/sessions/${activeSessionId}/artifacts`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (artData.data) {
          ...
          setStatus("idle");
          setMessages((prev) => [
            ...prev.filter((m) => m.id !== tempAsstMsg.id),
            {
              id: Date.now().toString(),
              role: "assistant",
              content: `Artifact generation failed: ${artData.error.message || "Unknown error"}`,
            },
          ]);
        } else if (!artRes.ok) {
          setStatus("idle");
          setMessages((prev) => [
            ...prev.filter((m) => m.id !== tempAsstMsg.id),
            {
              id: Date.now().toString(),
              role: "assistant",
              content: `Artifact generation failed: ${artData.detail || `HTTP ${artRes.status}`}`,
            },
          ]);
        }
        fetchSessions();
      ...
      const response = await fetch(
        `${API_BASE}/sessions/${activeSessionId}/messages`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    <div className="app-container">
          ...
          {messages.length === 0 ? (
            <div className="welcome-container">
              <div className="welcome-title">
                Welcome to Lenny's Growth Assistant
              </div>
              <div className="welcome-subtitle">
                Grounded intelligence trained on 260+ Lenny's Podcast
                transcripts. Ask product, growth, and strategy questions to get
                cited answers from top operators.
              </div>
              <div className="prompt-grid">
                {SUGGESTED_PROMPTS.map((p, idx) => (
                  <div
                    ...
                    onClick={() => handleSend("answer_question", p.query)}
                  >
                  ...
                  <div className="citations-container" aria-label="Citations">
                    ...
                    <div className="citations-list">
                      {m.citations.map((c, cIdx) => (
                        ...
                        >
                          📍 {c.guest_name || "Guest"} —{" "}
                          {c.episode_title || "Episode"} (
                          {c.approx_timestamp || "00:00"})
                        </button>
                      ))}
                    </div>
                  </div>
        ...
        <div className="input-area-wrapper">
          <div className="input-area">
            <input
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && handleSend("answer_question")
              }
            />
            <button
              className="btn-send"
              onClick={() => handleSend("answer_question")}
            >
              Send
            </button>
            <button
              className="btn-essay"
              onClick={() => handleSend("write_ship30_essay")}
            >
              Ship 30 Essay
            </button>
          </div>
      ...
      {artifact && (
        <aside className="artifact-zone" aria-label="Artifact Viewer">
          ...
          >
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              <button
                ...
                onClick={() =>
                  setArtifactMode(
                    artifactMode === "rendered" ? "raw" : "rendered",
                  )
                }
              >
          ...
          >
            <span>
              Word Count: <strong>{artifact.word_count}</strong>
            </span>
            <span>
              Structure:{" "}
              <strong>
                {artifact.structure_valid ? "✅ PASS" : "❌ FAIL"}
              </strong>
            </span>
          </div>
            ...
            {artifactMode === "raw" ? (
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: parseMarkdownToHTML(artifact.content),
                }}
              />
            )}
          </div>
